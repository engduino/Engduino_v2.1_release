...
*
* This sends a single bit. A double mark is sent as a mark signal, and a space
* is sent as a space followed by a double mark. The double mark is used
* because the input from the RX process goes to drive the counter in a timer.
* Even set to one count the counter expires and is reset only once every two
* marks - i.e. we get one receive interrupt every two marks. This encoding
* works well because we do not have to concern ourselves about the relative
* timings of runs of spaces. Instead a mark - space transition should always
* be a constant length, and this should be considerably longer than a
